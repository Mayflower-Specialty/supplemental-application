...
Can generate both blank forms and filled examples.
"""
...
class CheckBox(Flowable):
    """A square checkbox."""
    def __init__(self, size=8.5, checked=False):
        super().__init__()
        self.size = size
        self.width = size
        self.height = size
    ...
    def draw(self):
        ...
        if self.checked:
            ...
            c.line(margin, margin, self.size - margin, self.size - margin)
            c.line(margin, self.size - margin, self.size - margin, margin)


class YesNoBoxes(Flowable):
    """Inline Yes [] No [] used at the right edge of question rows."""
    def __init__(self, width=70, height=10, answer=None):
        super().__init__()
        self.width = width
        self.height = height
        self.answer = answer  # "Yes", "No", or None
    ...
    def draw(self):
        ...
        if self.answer == "Yes":
            ...
            c.setLineWidth(1.2)
            margin = 1.7
            c.line(18 + margin, 0.5 + margin, 18 + 8.5 - margin, 0.5 + 8.5 - margin)
            c.line(18 + margin, 0.5 + 8.5 - margin, 18 + 8.5 - margin, 0.5 + margin)
        ...
        if self.answer == "No":
            ...
            c.setLineWidth(1.2)
            margin = 1.7
            c.line(50 + margin, 0.5 + margin, 50 + 8.5 - margin, 0.5 + 8.5 - margin)
            c.line(50 + margin, 0.5 + 8.5 - margin, 50 + 8.5 - margin, 0.5 + margin)


class FieldRow(Flowable):
    """A labeled blank field: small teal label sitting above an underline rule."""
    def __init__(self, label, width, value=""):
        super().__init__()
        self.label = label
        self.width = width
        self.value = value
    ...
    def draw(self):
        ...
        if self.value:
            ...
            c.setFillColor(colors.black)
            c.drawString(0, self.height - 20, self.value)
...
class SignatureFieldRow(Flowable):
    ...
    def draw(self):
        ...
        # Draw value in italic script-like font if provided
        if self.value:
            c.setFont("Helvetica-Oblique", 10)
            c.setFillColor(colors.HexColor("#1a2a3a"))
            c.drawString(0, self.height - 24, self.value)
...
class AnswerBox(Flowable):
    ...
    def __init__(self, width, lines=2, text=""):
        super().__init__()
        self.width = width
        self.lines = lines
        self.line_h = 11
    ...
    def draw(self):
        ...
        if self.text:
            c.setFillColor(colors.HexColor("#2a3a48"))
            c.setFont("Helvetica", 8.5)

            # Split text into lines that fit within the box width
            words = self.text.split()
            current_line = ""
            y_position = self.height - 10
            ...
            for word in words:
                ...
                if c.stringWidth(test_line, "Helvetica", 8.5) <= max_width:
                    ...
                    if current_line:
                        c.drawString(4, y_position, current_line)
                        y_position -= self.line_h
                        if y_position < 2:  # Don't overflow the box
                            break
                    current_line = word

            # Draw last line if there's room
            if current_line and y_position >= 2:
                c.drawString(4, y_position, current_line)
...
def options_grid(items, cols=2, col_widths=None, bold=False, selections=None):
    """
    Build a Table of checkbox options.
    items: list of either str (label) or tuple (label, helper)
    selections: list of indices (0-based) that should be checked, or None for all unchecked
    """
    ...
    for idx, it in enumerate(items):
        ...
        cur.append(CheckBox(checked=(idx in selections)))
        cur.append(para)
        if len(cur) == cols * 2:
            rows.append(cur)
            cur = []
...
def question(num, text, helper=None, yes_no=False, answer=None):
...
def fields_row(items):
    ...
    items: list of (label, weight) or (label, weight, value) tuples.
    """
    # Normalize items to always have value (default empty string)
    normalized = []
    for item in items:
        if len(item) == 2:
            label, weight = item
            ...
            label, weight, value = item
        normalized.append((label, weight, value))

    total = sum(w for _, w, _ in normalized)
    col_widths = [(CONTENT_W * (w / total)) for _, w, _ in normalized]

    # Build cells with exact col widths minus right padding
    cells = [FieldRow(label, cw - 6, value) for (label, _, value), cw in zip(normalized, col_widths)]
...
def build_story(data=None):
    """
    Build the complete story (list of flowables) for the application.

    Args:
        data: Optional dictionary with filled data. If None, generates blank form.
    ...
    def get(key, default=""):
        """Get a value from data dict, or return default if data is None or key missing."""
        if data is None:
            return default
        return data.get(key, default)

    def get_list(key, default=None):
        """Get a list from data dict, or return empty list/None if data is None or key missing."""
        if data is None:
            return default
        return data.get(key, default)
    ...
    story.append(options_grid([
        ...
        "AI DIC Excess",
    ], cols=2, selections=get_list("coverage_modules")))
    story.append(Spacer(1, 8))
    ...
    # Field rows
    story.append(FieldRow("1. Named Insured (full legal name, including all subsidiaries for which coverage is requested)", CONTENT_W, get("named_insured")))
    story.append(Spacer(1, 4))
    story.append(fields_row([("2. DBA (if any)", 1, get("dba")), ("3. State of Incorporation / Formation", 1, get("state_incorporation"))]))
    story.append(Spacer(1, 4))
    story.append(FieldRow("4. Principal Office Address", CONTENT_W, get("address")))
    story.append(Spacer(1, 4))
    story.append(fields_row([("5. City", 2, get("city")), ("State", 1, get("state")), ("Zip", 1, get("zip"))]))
    story.append(Spacer(1, 4))
    story.append(fields_row([("6. Phone", 1, get("phone")), ("Email", 1, get("email")), ("Website", 1, get("website"))]))
    story.append(Spacer(1, 4))
    story.append(fields_row([("7. Industry / Sector", 2, get("industry")), ("8. NAICS Code", 1, get("naics")), ("9. Year Founded", 1, get("year_founded"))]))
    story.append(Spacer(1, 4))
    story.append(fields_row([("10. Annual Revenue (USD)", 1, get("annual_revenue")), ("11. Total Employees", 1, get("total_employees")), ("12. Number of Locations", 1, get("num_locations"))]))
    story.append(Spacer(1, 4))

    story.append(fields_row([("13. Requested Policy Period", 1, get("policy_period")), ("14. Requested Aggregate Limit", 1, get("aggregate_limit"))]))
    story.append(Spacer(1, 4))

    story.append(fields_row([("15. Contact Name and Title", 1, get("contact_name")), ("16. Contact Email and Phone", 1, get("contact_email_phone"))]))
    story.append(Spacer(1, 4))
    ...
    ], cols=4, selections=get_list("ownership_structure")))
    story.append(Spacer(1, 6))
    ...
    ], cols=4, selections=get_list("business_type")))
    story.append(Spacer(1, 6))
    ...
    ], cols=3, selections=get_list("geographic_scope")))
    ...
    ], cols=3, bold=True, selections=get_list("q2_1_systems")))
    story.append(Spacer(1, 6))
    ...
    story.append(AnswerBox(CONTENT_W, lines=3, text=get("q2_2_schedule")))
    story.append(Spacer(1, 8))
    ...
    ], cols=1, bold=True, selections=get_list("q2_3_integration")))
    story.append(Spacer(1, 6))
    ...
    ], cols=1, bold=True, selections=get_list("q2_4_materiality")))
    story.append(Spacer(1, 6))
    ...
    story.append(FieldRow("", CONTENT_W, get("q2_5a_decisions")))
    story.append(Spacer(1, 2))
    ...
    story.append(FieldRow("", CONTENT_W, get("q2_5b_individuals")))
    story.append(Spacer(1, 2))
    story.append(Paragraph("<b>(c)</b> Annual revenue derived from AI-enabled products or services (USD):", QUESTION))
    story.append(FieldRow("", CONTENT_W, get("q2_5c_revenue")))
    story.append(Spacer(1, 2))
    story.append(Paragraph("<b>(d)</b> Percentage of AI usage that is:", QUESTION))

    # Handle q2_5d_usage - it's a list of tuples for fields_row or empty list
    usage_data = get_list("q2_5d_usage", default=[])
    if usage_data:
        story.append(fields_row(usage_data))
    else:
    ...
    ], cols=1, bold=True, selections=get_list("q2_6_deployment")))
    story.append(Spacer(1, 8))
    ...
    ], cols=2, bold=True, selections=get_list("q2_7_generative")))
    story.append(Spacer(1, 4))
    ...
    story.append(AnswerBox(CONTENT_W, lines=2, text=get("q2_7a_models")))
    story.append(Spacer(1, 6))
    ...
    story.append(options_grid([
        ...
        "None of the above",
    ], cols=2, selections=get_list("q2_7b_guardrails")))
    story.append(Spacer(1, 6))
    ...
    ], cols=1, bold=True, selections=get_list("q2_8_third_party")))
    story.append(Spacer(1, 4))
    ...
    ], cols=2, selections=get_list("q2_8a_vendor")))
    ...
    story.extend(question(9, "Does any AI system operate fully autonomously without real-time human intervention capability?",
                          ...
                          yes_no=True, answer=get("q2_9_autonomous")))
    story.append(Spacer(1, 4))
    ...
    story.append(AnswerBox(CONTENT_W, lines=3, text=get("q2_9a_autonomous_desc")))
    story.append(Spacer(1, 6))
    ...
    ], cols=2, bold=True, selections=get_list("q2_10_agents")))
    story.append(Spacer(1, 6))
    ...
    story.append(AnswerBox(CONTENT_W, lines=2, text=get("q2_10a_agents_desc")))
    story.append(Spacer(1, 6))
    ...
    story.append(options_grid([
        ...
        "None of the above",
    ], cols=2, selections=get_list("q2_11_prohibited")))
    story.append(Spacer(1, 6))
    ...
    ], cols=1, selections=get_list("q3_1_framework")))
    story.append(Spacer(1, 6))
    ...
    story.append(options_grid([
        ...
        "Other",
    ], cols=2, selections=get_list("q3_2_frameworks")))
    story.append(Spacer(1, 2))
    story.append(FieldRow("If Other, specify", CONTENT_W - 30, get("q3_2_other")))
    story.append(Spacer(1, 6))
    ...
    ], cols=2, bold=True, selections=get_list("q3_3_oversight")))
    story.append(Spacer(1, 4))
    ...
    story.append(AnswerBox(CONTENT_W, lines=2, text=get("q3_3a_oversight_desc")))
    story.append(Spacer(1, 6))
    ...
    ], cols=2, bold=True, selections=get_list("q3_4_board_reporting")))
    story.append(Spacer(1, 6))
    ...
    story.append(options_grid([
        ...
        ("No", "no such policy"),
    ], cols=1, bold=True, selections=get_list("q3_5_ethics_policy")))
    story.append(Spacer(1, 6))
    ...
    story.append(options_grid([
        ...
        ("No", None),
    ], cols=2, bold=True, selections=get_list("q3_6_standards")))
    story.append(Spacer(1, 6))
    ...
    ], cols=1, bold=True, selections=get_list("q3_7_documentation")))
    ...
    story.append(options_grid([
        ...
        ("In development", None),
        ("No", "black-box only"),
    ], cols=2, bold=True, selections=get_list("q3_8_xai")))
    story.append(Spacer(1, 2))
    story.append(AnswerBox(CONTENT_W, lines=2, text=get("q3_8_xai_desc")))
    story.append(Spacer(1, 6))
    ...
    ], cols=2, bold=True, selections=get_list("q3_9_human_oversight")))
    story.append(Spacer(1, 6))
    ...
    ], cols=2, bold=True, selections=get_list("q3_10_vendor_mgmt")))
    story.append(Spacer(1, 6))
    ...
    story.append(options_grid([
        ...
        ("No", None),
    ], cols=2, bold=True, selections=get_list("q3_11_change_mgmt")))
    story.append(Spacer(1, 6))
    ...
    story.append(options_grid([
        ...
        "None of the above",
    ], cols=2, selections=get_list("q3_12_predeployment")))
    story.append(Spacer(1, 6))
    ...
    story.append(options_grid([
        ...
        ("In development", None),
        ("No", None),
    ], cols=2, bold=True, selections=get_list("q3_13_training")))
    story.append(Spacer(1, 6))
    ...
    ], cols=2, selections=get_list("q3_14_accountabilities")))
    ...
    ], cols=1, bold=True, selections=get_list("q4_1_lineage")))
    story.append(Spacer(1, 6))
    ...
    story.append(options_grid([
        ...
        ("None", None),
    ], cols=2, bold=True, selections=get_list("q4_2_quality")))
    story.append(Spacer(1, 6))
    ...
    story.append(options_grid([
        ...
        ("No", None),
    ], cols=2, bold=True, selections=get_list("q4_3_bias")))
    story.append(Spacer(1, 4))
    ...
    story.append(AnswerBox(CONTENT_W, lines=2, text=get("q4_3a_bias_desc")))
    story.append(Spacer(1, 6))
    ...
    story.append(options_grid([
        ...
        "None of the above",
    ], cols=2, selections=get_list("q4_4_data_categories")))
    story.append(Spacer(1, 6))
    ...
    ], cols=2, bold=True, selections=get_list("q4_5_compliance")))
    story.append(Spacer(1, 6))
    ...
    story.append(options_grid([
        ...
        "None of the above",
    ], cols=2, selections=get_list("q4_5a_regulations")))
    story.append(Spacer(1, 6))
    ...
    _q6_block.append(options_grid([
        ...
        ("In development", None),
        ("No", None),
    ], cols=2, bold=True, selections=get_list("q4_6_retention")))
    ...
    _q7_block.append(options_grid([
        ...
        "None of the above",
    ], cols=1, selections=get_list("q4_7_provenance")))
    ...
    ], cols=2, bold=True, selections=get_list("q4_8_subject_rights")))
    ...
    ], cols=1, bold=True, selections=get_list("q5_1_criticality")))
    story.append(Spacer(1, 6))
    ...
    ], cols=2, bold=True, selections=get_list("q5_2_external_infra")))
    story.append(Spacer(1, 6))
    ...
    story.append(options_grid([
        ...
        ("No", None),
    ], cols=3, bold=True, selections=get_list("q5_3_fallback")))
    story.append(Spacer(1, 6))
    ...
    story.append(options_grid([
        ...
        ("No", None),
    ], cols=2, bold=True, selections=get_list("q5_4_monitoring")))
    story.append(Spacer(1, 6))
    ...
    ], cols=2, bold=True, selections=get_list("q5_5_retrain")))
    story.append(Spacer(1, 6))
    ...
    story.append(options_grid([
        ...
        ("No", "direct cutover"),
    ], cols=2, bold=True, selections=get_list("q5_6_testing")))
    story.append(Spacer(1, 6))
    ...
    ], cols=2, bold=True, selections=get_list("q5_7_override_logging")))
    story.append(Spacer(1, 8))
    ...
    story.append(options_grid([
        ...
        ("No", None),
    ], cols=2, bold=True, selections=get_list("q6_1_ir_plan")))
    story.append(Spacer(1, 6))
    ...
    ], cols=1, selections=get_list("q6_2_detection")))
    story.append(Spacer(1, 6))
    ...
    story.append(options_grid([
        ...
        ("Yes, one", None),
        ("No", None),
    ], cols=3, bold=True, selections=get_list("q6_3_tabletop")))
    story.append(Spacer(1, 6))
    ...
    story.append(options_grid([
        ...
        ("No", None),
    ], cols=1, bold=True, selections=get_list("q6_4_complaint")))
    story.append(Spacer(1, 6))
    ...
    ], cols=2, bold=True, selections=get_list("q6_5_appeal")))
    story.append(Spacer(1, 6))
    ...
    story.append(options_grid([
        ...
        ("Ad hoc", None),
        ("No", None),
    ], cols=2, bold=True, selections=get_list("q6_6_rca")))
    story.append(Spacer(1, 6))
    ...
    ], cols=4, bold=True, selections=get_list("q6_7_incidents")))
    story.append(Spacer(1, 4))
    ...
    story.append(AnswerBox(CONTENT_W, lines=2, text=get("q6_7a_incident_desc")))
    ...
    ], cols=2, bold=True, selections=get_list("q7_1_eu_ai_act")))
    story.append(Spacer(1, 6))
    ...
    story.append(options_grid([
        ...
        "Other",
        "None or Unsure",
    ], cols=2, selections=get_list("q7_2_us_regs")))
    story.append(Spacer(1, 6))
    ...
    story.append(options_grid([
        ...
        ("No", None),
        ("N/A", None),
    ], cols=2, bold=True, selections=get_list("q7_3_sector_regs")))
    story.append(Spacer(1, 4))

    # Q3a
    story.append(Paragraph("<b>3a.</b>  If Yes or Partial, identify regulator(s) and describe the compliance program.", QUESTION))
    story.append(AnswerBox(CONTENT_W, lines=2, text=get("q7_3a_sector_desc")))
    story.append(Spacer(1, 6))
    ...
    story.append(options_grid([
        ...
        ("No", None),
    ], cols=2, bold=True, selections=get_list("q7_4_external_audit")))
    story.append(Spacer(1, 6))
    ...
    story.append(options_grid([
        ...
        ("No", None),
    ], cols=2, bold=True, selections=get_list("q7_5_iso_42001")))
    story.append(Spacer(1, 6))
    ...
    story.extend(question(6, "Regulatory inquiries, investigations, consent decrees, or enforcement actions relating to AI or algorithmic systems in the past 3 years.",
                          ...
                          yes_no=True, answer=get("q7_6_regulatory_inquiries")))
    story.append(Spacer(1, 4))

    # Q6a
    story.append(Paragraph("<b>6a.</b>  If yes, provide regulator, date, subject matter, status, and any remedial terms.", QUESTION))
    story.append(AnswerBox(CONTENT_W, lines=2, text=get("q7_6a_inquiries_desc")))
    story.append(Spacer(1, 8))
    ...
    # Q1 (Yes/No)
    story.extend(question(1, "Any claim, suit, regulatory action, or complaint arising from an AI or algorithmic system in the past three years?", yes_no=True, answer=get("q8_1_ai_claims")))
    story.append(Paragraph("<b>1a.</b>  If yes, provide details (date, nature, status, amount paid or reserved).", QUESTION))
    story.append(AnswerBox(CONTENT_W, lines=2, text=get("q8_1a_ai_claims_desc")))
    story.append(Spacer(1, 6))

    # Q2
    story.extend(question(2, "Any D&amp;O, EPL, or E&amp;O claim or suit in the past three years, regardless of AI involvement?", yes_no=True, answer=get("q8_2_other_claims")))
    story.append(Paragraph("<b>2a.</b>  If yes, provide details (date, line, nature, status, amount).", QUESTION))
    story.append(AnswerBox(CONTENT_W, lines=2, text=get("q8_2a_other_claims_desc")))
    story.append(Spacer(1, 6))

    # Q3
    story.extend(question(3, "Aware of any facts or circumstances that could reasonably give rise to a claim?", yes_no=True, answer=get("q8_3_circumstances")))
    story.append(Paragraph("<b>3a.</b>  If yes, provide details.", QUESTION))
    story.append(AnswerBox(CONTENT_W, lines=2, text=get("q8_3a_circumstances_desc")))
    story.append(Spacer(1, 6))

    # Q4
    story.extend(question(4, "Has any insurer declined, cancelled, or non-renewed coverage for the Applicant?", yes_no=True, answer=get("q8_4_declined")))
    story.append(Paragraph("<b>4a.</b>  If yes, provide details.", QUESTION))
    story.append(AnswerBox(CONTENT_W, lines=2, text=get("q8_4a_declined_desc")))
    ...
    empty_row = ["", "", "", "", "", ""]
    prior = get_list("prior_insurance", default=[])
    filled_rows = [list(row) for row in prior] if prior else []
    blank_rows = [empty_row] * max(0, 4 - len(filled_rows))
    ins_data = [ins_headers] + filled_rows + blank_rows
    col_w = CONTENT_W / 6
    ...
    ins_table = Table(ins_data, colWidths=ins_widths, rowHeights=[20] + [26] * (len(ins_data) - 1))
    ...
    story.append(Spacer(1, 8))
    story.append(fields_row([("Proposed Retroactive Date", 1, get("retro_date")), ("Current Carrier Expiration Date", 1, get("expiration_date"))]))
    ...
    sig_grid = Table([
        [SignatureFieldRow("Signature of Authorized Representative", sig_widths[0] - 12, get("signature")), SignatureFieldRow("Date", sig_widths[1] - 12, get("signature_date"))],
        [Spacer(1, 8), Spacer(1, 8)],
        [SignatureFieldRow("Printed Name", sig_widths[0] - 12, get("printed_name")), SignatureFieldRow("Title", sig_widths[1] - 12, get("title"))],
        [Spacer(1, 8), Spacer(1, 8)],
        [SignatureFieldRow("Organization", sig_widths[0] - 12, get("organization")), SignatureFieldRow("Email", sig_widths[1] - 12, get("sig_email"))],
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Generate Mayflower AI Liability Supplemental Application PDF")
    parser.add_argument("--filled", action="store_true",
                        help="Generate filled example using mock data instead of blank form")
    args = parser.parse_args()

    # Import mock data if --filled flag is passed
    data = None
    if args.filled:
        from mock_data import MOCK_DATA
        data = MOCK_DATA

    # Build the story with optional data
    story = build_story(data)

    # Determine output filename
    if args.filled:
        OUT = "pdfs/Mayflower_AI_Liability_Supplemental_Application_v6_FILLED_EXAMPLE.pdf"
    else:
        OUT = "pdfs/Mayflower_AI_Liability_Supplemental_Application_v6.pdf"
    ...
    doc.build(story)
    mode = "filled example" if args.filled else "blank form"
